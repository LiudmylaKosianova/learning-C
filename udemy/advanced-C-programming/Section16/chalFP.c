...
int* performOp(int *a, int *b, int size, MFunc *FPtr/* ADD FUNCTION POINTER PARAMETER HERE */);
...
int main( ){
    ...
    MFunc *multPtr = mult;
    MFunc *divPtr = divv;
    MFunc *FPtrArr[] = {addPtr, subPtr, multPtr,divPtr};
  ...
  while(choice != 5)
  {
	...
	result = performOp(array1,array2,size, FPtrArr[j]/* PASS IN FUNCTION POINTER FROM ARRAY[j] */);		
